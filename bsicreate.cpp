...
int main() {
	...
	if(os == 1) {
		...
		sprintf(com1, "cd /home/artem && mkdir %s && cd %s && truncate -s %iG %s.img", name, name, disk, name);
		//system(com1);
		sprintf(com2, " && echo '#!/bin/sh /usr/share/examples/bhyve/vmrun.sh -c %i -m %i -t %s -d %s.img %s' > start.sh && chmod +x start.sh", cpu, mem, tap, name, name);
